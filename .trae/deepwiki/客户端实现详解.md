...
该架构设计的核心思想是**协议封装**。通过将 SSH 流量封装在 WebSocket 中，客户端可以轻易穿透防火墙和代理服务器，因为 WebSocket 流量在外观上与普通的 HTTPS 流量无异。`client` 结构体作为控制器，通过 `sessions map[int64]*session.BidirectionalSession` 管理多会话，并利用 `Interpreter` 获取系统上下文。

**架构设计要点**：
- **解耦**：CLI 解析与核心运行逻辑分离，方便嵌入到其他程序中。
- **并发**：每个连接和心跳任务都在独立的 goroutine 中运行，互不干扰。
- **状态追踪**：客户端使用受 `sessionTrackMutex` 保护的 `sessions` 映射记录活动会话，支持 Listener/Beacon 场景下的多连接和断线清理。
...
### 会话追踪 (sessions map)
为了支持多连接（特别是在 Listener 和 Beacon 模式下），客户端使用 `sessions map[int64]*session.BidirectionalSession` 来管理所有活动会话。

```go
sessions map[int64]*session.BidirectionalSession
```

每个会话都有一个唯一的 ID，通过 `sessionTrackMutex` 保证并发安全。当连接断开时，`dropWebSocketSession` 会先调用 `sess.Close()` 释放底层资源，再从映射中删除该会话。

### 解释器 (Interpreter)
`interpreter` 组件负责在连接建立时收集客户端元数据，包括操作系统、架构、当前用户、主机名、工作目录和进程诊断信息。`ProcessInfo` 只包含经过清洗的进程名和 PID，并作为非可信诊断信息随 `client-info` 请求上报给服务端。
...
### 3. 应用层握手
连接建立后的第一件事是调用 `sendClientInfo`。客户端将 `interpreter.Info` 序列化为 JSON，通过 `slider-client-info` 请求发送，其中包含 `BaseInfo` 和可选的 `ProcessInfo`。服务端收到后会返回其自身的系统信息、进程诊断和 `fingerprint:port` 身份，完成双向身份确认并为网格环路检测提供依据。
...
### 命令行参数分类
1. **基础连接**：`server_address`（位置参数）、`--key`（SSH 身份私钥）、`--fingerprint`（服务端 SSH 指纹）、`--dns`（自定义解析）。
2. **模式切换**：`--listener`、`--beacon`。
3. **隐蔽性配置**：`--http-template`、`--http-server-header`、`--proto`（自定义 WebSocket 子协议）。
4. **网络维护**：`--keepalive`、`--retry`。
5. **TLS 验证**：`--server-ca` 指定 HTTPS 服务端 CA，`--server-name` 指定 TLS SNI/主机名校验值，`--tls-cert` 与 `--tls-key` 提供客户端 TLS 证书。

### 互斥校验
`cli.go` 中定义了严格的标志位校验逻辑。例如，`--listener` 模式与 `--retry`、`--key`、`--dns`、`--tls-cert`、`--tls-key`、`--server-ca`、`--server-name` 是互斥的，因为在被动监听模式下，重连和主动出站 TLS 验证逻辑不适用。
...
客户端支持从字符串或文件中加载多个合法的指纹。如果配置了指纹，`sshConfig.HostKeyCallback` 会被设置为 `verifyServerKey`，在 SSH 握手阶段强制执行校验。当前实现还增加了传输层约束：连接 `https://` 服务端时使用系统根证书或 `--server-ca` 指定的 CA 验证 TLS，`--server-name` 可覆盖 TLS 主机名；连接明文 `http://` 服务端时必须提供 `--fingerprint`，避免 HTTP 和 SSH 两层都缺失对端身份校验。
...
- [pkg/interpreter/process.go](pkg/interpreter/process.go): 进程诊断信息采集与清洗。
