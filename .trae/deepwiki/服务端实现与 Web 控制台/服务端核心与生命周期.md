...
- **子模块**:
  ...
  - `web/`: 包含 Web 控制台的 Vite/TypeScript 源码。
  - `server/web/dist`: 包含由 `go:embed` 打进服务端二进制的前端构建产物。
- **核心职责**:
...
type server struct {
	*slog.Logger                          // 统一日志记录器
	...
	remoteSessions       map[remoteStateKey]*RemoteSessionState // 远程 endpoint 状态映射
	unifiedSessionIDs    map[SessionKey]int64 // 远程会话稳定 ID 映射
	localSocks           LocalSocksServer // 独立本地 SOCKS5 服务状态
}
```

每个字段都承载着特定的职责。例如，`sessionTrack` 负责监控连接的存活，`commandRegistry` 定义本地和 Web 控制台共用的命令集合，`remoteSessions` 以 `remoteStateKey` 区分远程会话上的 SSH、Shell、SOCKS 和端口转发状态。通过将这些组件集成在一个结构体中，Slider 实现了高效的状态共享和方法调用。

### 并发模型与同步机制

由于服务端需要同时处理大量的并发连接和控制台输入，并发安全至关重要。Slider 采用了典型的 Go 并发模式：

1. **互斥锁保护**: 对于 `Sessions`、`CertTrack`、`remoteSessions` 和 `unifiedSessionIDs` 等映射，服务端使用 `sync.Mutex` 或 `sync.RWMutex`。在添加、删除或查询共享状态时，必须获取相应的锁。
2. **全局会话 ID**: Session ID 的生成和活跃计数由 `pkg/session` 统一维护；服务端自身只保存当前本地会话映射，并通过 `session_resolver.go` 为远程会话分配稳定的统一 ID。
...
上图展示了服务端的并发结构。主 HTTP listener 运行在独立 goroutine 中，WebSocket 升级后再进入 SSH server/client 处理流程。所有的处理 goroutine 共享受 `sync.RWMutex` 保护的会话映射表；远程 endpoint 状态也通过独立互斥锁保护，避免会话列表、远程 SOCKS/Shell/SSH 状态互相踩踏。
...
1. **配置对象组装**: `server/config.go` 的 `RunServer` 接收 CLI 层传入的 `Config`。
2. **服务端初始化**: `newConfiguredServer` 创建 logger、解释器、证书库、命令注册表和远程会话状态。
3. **身份标识生成**: `configureServerIdentity` 生成或加载服务端 Ed25519 密钥对，并在开启 `--auth` 时加载 `client-certs.json`。
4. **HTTP 参数校验**: `configureServerHTTP` 校验模板、状态码、keepalive 和 redirect 配置。
5. **监听器启动**: `startHTTPListener` 启动 HTTP 或 HTTPS listener，并把请求交给 `buildRouter`。
6. **可选 Callback**: `startStartupCallback` 在 `--gateway --auth --callback-cert-id` 条件满足时发起上游回调连接。
7. **运行阻塞**: `runUntilShutdown` 根据 Headless 与否进入信号等待或本地交互控制台循环。
...
sequenceDiagram
    ...
    RS->>CFG: newConfiguredServer(cfg)
    CFG->>CERT: 生成/加载服务端 Ed25519 密钥
    Note over CERT: 确定服务端指纹 (Fingerprint)
    CFG->>CERT: 加载客户端证书罐 (CertJar)
    RS->>LIS: startHTTPListener(cfg)
    RS->>RS: startStartupCallback(cfg)
...
**Section sources**:
- [server/config.go](server/config.go)
- [server/bootstrap.go](server/bootstrap.go)
- [server/runtime.go](server/runtime.go)
...
服务端在 `server/handler_console_ws.go` 中创建 PTY 对，并在两个 goroutine 中分别处理 WebSocket -> PTY 和 PTY -> WebSocket 的字节流。前端的 `web/src/console.ts` 使用 `@xterm/xterm` 渲染 ANSI 输出，并把 resize 控制消息发送回后端同步 PTY 尺寸。
...
- [server/bootstrap.go](server/bootstrap.go): 服务端初始化、日志、证书与 HTTP 配置装配。
- [server/runtime.go](server/runtime.go): HTTP/HTTPS listener、Callback 和运行循环。
- [server/gateway.go](server/gateway.go): 网关模式、链式连接与循环检测实现。
- [server/handler.go](server/handler.go): HTTP 路由与 Agent/Gateway WebSocket 连接处理。
- [server/handler_console_ws.go](server/handler_console_ws.go): Web Console WebSocket、PTY 桥接和 resize 同步。
