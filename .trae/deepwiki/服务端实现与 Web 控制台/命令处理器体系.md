...
本模块涵盖了 Slider 服务端的所有管理命令实现，主要位于 `server/` 目录下。通过对该目录的扫描，共发现 23 个与命令处理直接相关的 Go 源文件。

- **总文件数**: 23 个
- **子模块划分**:
  - **基础框架**: `command.go`, `commands_basic.go` (定义接口与注册机制)
  - **会话管理**: `commands_sessions.go`, `commands_connect.go` (多级会话调度)
  - **远程交互**: `commands_shell.go`, `console.go`, `console_out.go` (PTY 交互与 UI)
  - **SFTP 体系**: `command_sftp.go`, `console_sftp.go` 及所有 `commands_sftp_*.go` 文件 (文件管理)
  - **网络代理**: `commands_socks.go`, `commands_portfwd.go` (隧道与代理)
...
在服务器初始化阶段，`initRegistry` 方法会将所有内置命令注册到全局注册表中。对于 SFTP 这种特定场景，Slider 会创建一个独立的 `CommandRegistry` 实例，以隔离命令空间。
...
`ResolveUnifiedSessions` 函数会遍历所有本地直接连接的会话，并递归地向支持 Gateway 模式的会话发送查询请求，获取其下游的远程会话。
...
为了防止本地其他进程恶意连接到这个临时的 Shell 端口，Slider 强制要求使用 mTLS。
...
这些功能都是通过 `instance.Config` 组件管理的，它负责维护所有的端口映射关系和活跃的转发协程。
...
- **基础与会话命令**:
  ...
  - [server/commands_sessions.go](server/commands_sessions.go): 核心会话管理逻辑。
- **交互式功能**:
  - [server/commands_shell.go](server/commands_shell.go): 远程 Shell 与 mTLS 实现。
...
- **网络服务**:
  - [server/commands_socks.go](server/commands_socks.go): SOCKS5 代理管理。
