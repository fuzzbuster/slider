...
6. [连接状态与生命周期](#连接状态与生命周期)
   - [连接状态机](#连接状态机)
   - [会话组件关系](#会话组件关系)
7. [部署实践：控制面与数据面隔离](#部署实践控制面与数据面隔离)
8. [数据模型与协议定义](#数据模型与协议定义)
9. [文件参考](#文件参考)
...
在生产部署中，Slider 的控制面和数据面应被视为两个不同的安全域。控制面承载 `/auth`、`/console` 和 `/console/ws` 等人工操作入口；数据面承载 Agent、Gateway、Callback 等长期 WebSocket 会话。由于当前 Server 内部仍使用一个 HTTP(S) listener，并优先通过 `Sec-WebSocket-Protocol` 与 `Sec-WebSocket-Operation` 识别 Slider 数据会话，部署侧需要使用独立域名、CDN/WAF 规则、Origin 防直连和内层 SSH 指纹校验共同完成隔离。

推荐将不同 Gateway 按区域拆分为独立 listener 角色：核心 Gateway 接受 Operator 与 Callback，正向 Gateway 代表下游区域，Callback Gateway 主动回连核心入口。完整的 Cloudflare/CDN 配置、Slider 启动模板和验收清单见子章节 [控制面与数据面隔离最佳实践](核心模式与连接机制/控制面与数据面隔离最佳实践.md)。
