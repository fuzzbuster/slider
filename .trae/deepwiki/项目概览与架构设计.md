...
- **代码规模**：项目包含约 120 个 Go 源文件，并配套 Vite/TypeScript Web 控制台、Playwright 浏览器测试和多平台 CI 构建流程。
- **核心模块**：
    ...
    - `pkg/instance/`：提供 Shell、SOCKS5 代理、SSH endpoint、端口转发等具体功能的实例管理，并通过 `ServiceManager` 和 `endpointRun` 分离服务分发与监听生命周期。
    - `web/` 与 `server/web/dist`：前端源码由 Vite 构建，构建产物嵌入服务端二进制。
...
连接流程从基础的 HTTP 升级开始，确保了良好的防火墙兼容性。进入 SSH 协商阶段后，Slider 利用 Ed25519 密钥对完成身份验证，确保了连接的安全性。随后双方通过 `slider-client-info` 交换系统与进程诊断信息，并在加密的 SSH 隧道内开启 Shell、SFTP、SOCKS、端口转发或网格路由等功能通道。
...
当以 `--gateway` 模式启动 Server 时，它会开启路由转发功能。Gateway 节点通过 `slider-sessions` 递归发现下游会话，通过 `slider-connect` 和 `slider-forward-request` 转发控制请求。服务端侧的 `session_resolver.go` 会把本地和远程会话归一化为稳定的 `UnifiedID`，让用户通过一个入口点操作多级网络中的目标。
...
| `pkg/session/session.go` | 核心会话管理逻辑，定义 `BidirectionalSession`、角色状态和对端元数据。 |
| `server/session_resolver.go` | 将本地/远程会话归一化为稳定的统一会话视图。 |
| `pkg/instance/endpoint_run.go` | 管理 endpoint listener、活动连接和幂等停止流程。 |
...
| `web/src/console.ts` | Web 控制台 xterm 终端、WebSocket 和 resize 客户端逻辑。 |
