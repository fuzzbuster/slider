...
- **核心文件**：
  ...
  - `requests.go`: 处理 SSH 全局请求的顶层分发和基础请求。
  - `request_mesh.go`: 处理 `slider-sessions`、`slider-forward-request` 和 `slider-event` 等网格请求。
  - `request_forward_tcp.go` / `request_forward_udp.go`: 分别处理 TCP 与 UDP 反向转发请求。
...
type RemoteSession struct {
    ...
    Process           *interpreter.ProcessInfo
    Role           string
    WorkingDir     string
    IsConnector    bool
    IsGateway      bool
...
`Path` 字段是路由系统的核心，它记录了从当前节点到达目标节点所需经过的所有中间会话 ID。`Process` 字段只用于 `sessions`/`sysinfo` 等诊断展示，进入结构体前会经过 `interpreter.SanitizeProcessInfo` 清洗，不能作为路由或授权依据。
...
1.  **身份识别**：交换主机名、操作系统、用户、工作目录和经过清洗的进程诊断等元数据。
...
- [pkg/interpreter/process.go](pkg/interpreter/process.go)
...
全局请求处理 SSH 协议中的 `GlobalRequest`。这是 Slider 实现跨节点管理功能的关键。当前代码把请求族拆分到多个文件：`requests.go` 保留顶层分发和 keepalive/client-info/window-size/shutdown，`request_forward_tcp.go` 处理 TCP 反向转发，`request_forward_udp.go` 处理 UDP 反向转发，`request_mesh.go` 处理网格会话发现、跨节点请求转发和事件通知。
...
- [pkg/session/requests.go](pkg/session/requests.go)
- [pkg/session/request_forward_tcp.go](pkg/session/request_forward_tcp.go)
- [pkg/session/request_forward_udp.go](pkg/session/request_forward_udp.go)
- [pkg/session/request_mesh.go](pkg/session/request_mesh.go)
...
- `pkg/session/requests.go`: SSH 全局请求分发与基础请求处理。
- `pkg/session/request_forward_tcp.go`: TCP 反向端口转发请求处理。
- `pkg/session/request_forward_udp.go`: UDP 反向端口转发请求处理。
- `pkg/session/request_mesh.go`: 网格会话发现、跨节点请求转发和事件处理。
