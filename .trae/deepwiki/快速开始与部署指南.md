...
- **总文件数**: 约 120 个 Go 源文件，并包含 Vite/TypeScript Web 控制台源码、Playwright 浏览器测试和 CI/CD 配置。
- **核心子模块**:
...
- **覆盖深度**: 本指南将重点介绍 `client` 和 `server` 的部署与运行，深入分析 `pkg/scrypt` 中的证书处理逻辑，并涵盖 `Makefile`、`web/` 前端构建和 `.github/goreleaser.yml` 定义的发布流程。
...
### 编译环境要求
- **Go 版本**: 项目 `go.mod` 固定为 `go 1.25.0`，本地开发和 CI 均应使用该版本约束。
- **Node.js/npm**: Web 控制台由 Vite/TypeScript 构建；修改 `web/` 后需要运行 `npm run typecheck:web` 和 `npm run build:web`，并提交同步后的 `server/web/dist`。
...
构建流程首先通过 `go build` 命令并配合 `-trimpath -ldflags="-s -w"` 参数来移除路径、DWARF 调试信息和符号表，这是减小体积的第一步。接着，`Makefile` 会检查系统中是否存在 `upx`。如果存在，它将根据 `UPX_BRUTE` 变量决定压缩强度。最终生成的二进制文件存放在 `build/` 目录下，文件名格式为 `slider-<os>-<arch>`。Web 控制台构建产物位于 `server/web/dist`，`make web-check` 会在构建后检查该目录是否与源码同步。
...
Slider 使用 `GoReleaser` 配合 GitHub Actions 实现自动化构建与发布。这确保了每个版本的二进制文件都是在干净的环境中构建的，并经过了统一的优化处理。当前 release workflow 还会安装 Node 22、执行 `npm run typecheck:web` 与 `npm run build:web`，并校验 `server/web/dist` 是否已同步提交。
...
当开发者向仓库推送一个新的版本标签（如 `v1.0.0`）时，GitHub Actions 会启动。它首先配置 Go 和 Node 环境，下载 Go/npm 依赖，执行 Web 前端校验和 `go test -v ./...`，随后安装 `upx` 并调用 `goreleaser`，按照配置文件中的矩阵同时为 Linux、Windows 和 macOS 的多种架构（amd64, arm64, 386）生成二进制文件。`ldflags` 动态注入版本号、提交哈希、构建日期和 Go 版本。最后，所有构建好的文件会被打包并自动上传到 GitHub Release 页面。
...
- `server/server.go`: 服务端核心结构与 SSH 生命周期。
- `server/bootstrap.go`: 服务端初始化、证书和命令注册表装配。
- `server/runtime.go`: HTTP/HTTPS 监听器、Callback 和运行循环。
...
- `web/`: Web 控制台前端源码。
- `server/web/dist`: 嵌入服务端二进制的前端构建产物。
- `e2e/`: 原生与浏览器端到端测试。
