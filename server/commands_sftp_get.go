...
func (c *SftpGetCommand) Run(execCtx *ExecutionContext, args []string) error {
	...
	localPath := sftpCtx.GetLocalCwd()

	if !spath.IsAbs(sftpCtx.RemoteSystem(), remotePath) {
		remotePath = spath.Join(sftpCtx.RemoteSystem(), []string{sftpCtx.GetRemoteCwd(), remotePath})
	}
	...
	if rpFi.IsDir() {
		...
		wrdErr := sftpCtx.walkRemoteDir(remotePath, "", func(rPath, localRelPath string, isDir bool) error {
			...
			bytesWritten, cErr := sftpCtx.copyFileWithProgress(rFile, lFile, fileSize,
				fmt.Sprintf("Download (%d/%d)", currentFile, fileCount), ui)
			if cErr != nil {
